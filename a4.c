...
int copyfiles(int argc, char *argv[])
{
   ...
   for (i = 1; i < argc - 1; i++)
   {
      ...
      if (!child)
      {
         ...
         if (!isvalid(argv[i], dstpath)) write (pfd[1], &status, sizeof(status));
         else if (!makecp(argv[i], dstpath)) write (pfd[1], &status, sizeof(status));
         else
         {
            status |= (getpid() << 16) | (1 << 8) | (strlen(argv[i]));
   ...
   int count;
   unsigned int status;
   while ((count = read (pfd[0], &status, sizeof (status))) > 0)
   {
      if ((status<<16)>>24 == 1)
      {
         copies++;

         ssize_t count = read (pfd[0], buf, status & 0xff);
         if (count <= 0) die("read check2");

         buf[count] = '\0';

         fprintf(stderr, "%-10s: copied by process %d\n", buf, status>>16);
      }
   }
 }
   if (!copies) return 0;
